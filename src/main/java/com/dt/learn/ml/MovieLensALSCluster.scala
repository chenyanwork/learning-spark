...
object MovieLensALSCluster {
  ...
  def main(args: Array[String]) {
    ...
    //设置运行环境
    val conf = new SparkConf().setAppName("MovieLensALS").setMaster("spark://Master:7077")
    val sc = new SparkContext(conf)

    //装载用户评分，该评分由评分器生成
    val myRatings = loadRatings(sc,"/library/ml/mllib/als/als/personalRatings.txt")
    val myRatingsRDD = sc.parallelize(myRatings, 4)


    //样本数据目录
    val movieLensHomeDir = "/library/ml/mllib/als/als/movielens/large"
